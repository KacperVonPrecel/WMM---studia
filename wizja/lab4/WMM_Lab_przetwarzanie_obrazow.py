...
image = cv2.imread(data_dir+"anemone_col.png", cv2.IMREAD_GRAYSCALE)
...
def solaryzacja_m2(img):
    ...
    for h in range(0, img.shape[0]):
        for w in range(0, img.shape[1]):
            val = img[h, w]
            if val <= 127:
                solarizedimg_m2[h, w] = 2 * val
            else:
                solarizedimg_m2[h, w] = 2*(255-val)
...
image_noise = cv2.imread(data_dir+"anemone_col_inoise.png", cv2.IMREAD_UNCHANGED)
...
image_inoise = cv2.imread(data_dir+"anemone_col_inoise copy.png", cv2.IMREAD_UNCHANGED)
...
image = cv2.imread(data_dir+"anemone_col.png", cv2.IMREAD_UNCHANGED)
...
image = cv2.imread(data_dir+"lena_mono.png", cv2.IMREAD_GRAYSCALE)
    ...
    for x in range(0, sz, 1):
        for y in range(0, sz, 1):
            ...
            if dist > rad:
                ...
                img_hsv[y, x, 0] = hue                   ### hue: zależnie od kąta, od 0 do 179 (180 to znowu 0)
                img_hsv[y, x, 1] = np.round(dist/rad*255)  ### nasycenie: na brzegu max (255), maleje do 0 w środku
                img_hsv[y, x, 2] = 255                     ### value: zawsze max
